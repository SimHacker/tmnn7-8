...
#else
    onintr(); oops();	/* sigh...the lengths we go to to squelch lint! */
#endif /* !lint */
    ...
    *filelist = NULL;
	...
	for (fileptr = filelist; *fileptr; fileptr++) {
		...
		if (strcmp(*fileptr + strlen(*fileptr) - 2, ".Z") != 0) {
		    ...
		    if (strlen(cp) > 12) {
			(void) fprintf(stderr,"%s.Z: No such file or directory\n",cp);
			continue;
		    }
		...
		if (stat(ofname, &statbuf) == 0) {
		    ...
		    if (foreground()) {
			(void) fprintf(stderr," OK to overwrite (y or n)? ");
			(void) fflush(stderr);
...
#ifndef COMPATIBLE
    if (nomagic == 0) {
	(void) putchar((char)magic_header[0]);
	(void) putchar((char)magic_header[1]);
...
getcode() {
    ...
    static int goffset = 0, size = 0;
    static char_type gbuf[BITS];
    register int r_off, bits;
    register char_type *bp = gbuf;

    if ( clear_flg > 0 || goffset >= size || free_ent > maxcode ) {
	/*
	 * If the next entry will be too big for the current code
	 * size, then we must increase the size.  This implies reading
	 * a new gbuffer full, too.
	 */
	if ( free_ent > maxcode ) {
	    n_bits++;
	    if ( n_bits == maxbits )
	...
	size = fread((char *)gbuf, 1, n_bits, stdin );
	if ( size <= 0 )
...
#endif /* NO_UCHAR */
	bits -= (8 - r_off);
	r_off = 8 - r_off;		/* now, goffset into code word */
	/* Get any 8 bit parts in the middle (<=1 for up to 16 bits). */
	if ( bits >= 8 ) {
#ifdef NO_UCHAR
	    code |= (*bp++ & 0xff) << r_off;
...
#ifndef lint
	if (chmod(oname, mode))		/* Copy modes */
	    perror(oname);
	(void) chown(oname, statbuf.st_uid, statbuf.st_gid);	/* Copy ownership */
#else
	(void) chmod(oname, mode);
#endif /* lint */
	timep[0] = statbuf.st_atime;
	timep[1] = statbuf.st_mtime;
#ifndef lint
	(void) utime(oname, timep);	/* Update last accessed and modified times */
#endif /* !lint */
	valid = 0;	/* added by res -- prevents latent oname removal */
#ifndef lint
	if (unlink(ifname))	/* Remove input file */
	    perror(ifname);
#endif /* !lint */
	if(!quiet)
		(void) fprintf(stderr, " -- replaced with %s", oname);
...
cl_hash(hsiz)		/* reset code table */
	register count_int hsiz;
{
#ifndef XENIX_16	/* Normal machine */
	register count_int *htab_p = htab+hsiz;
#else
	register j;
	register long k = hsiz;
