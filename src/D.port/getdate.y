...
%{
	...
	/*	Quickdate entry point added by Eric Raymond	*/
...
tspec:	NUMBER MERIDIAN =
	...
	| NUMBER ':' NUMBER MERIDIAN =
  	...
  	| NUMBER ':' NUMBER NUMBER =
  		{hh = $1; mm = $3; merid = 24;
 		sdaylight = STANDARD; ourzone = -($4%100 + 60*($4/100));}
	| NUMBER ':' NUMBER ':' NUMBER =
	...
	| NUMBER ':' NUMBER ':' NUMBER MERIDIAN =
  	...
  	| NUMBER ':' NUMBER ':' NUMBER NUMBER =
  		{
		    hh = $1; mm = $3; ss = $5; merid = 24;
		    sdaylight = STANDARD; ourzone = $6%100 + 60*$6/100;
		}
	;
...
static time_t dateconv(mm, dd, yy, h, m, s, mer, zone, dayflag)
...
static time_t dayconv(ord, day, now)
int ord, day; time_t now;
{
    ...
    tod += 7L * daysec*(ord<=0?ord:ord-1);
    return(daylcorr(tod, now));
}

static time_t timeconv(hh, mm, ss, mer)
...
static time_t monthadd(sdate, relmonth)
...
static time_t daylcorr(future, now)
time_t future, now;
{
    int fdayl, nowdayl;
...
time_t getdate(p, now)
char *p; struct timeb *now;
{
    time_t monthadd();
    int err;
    struct tm *lt;
    ...
    if (err = yyparse() || timeflag>1 || zoneflag>1 || dateflag>1 || dayflag>1)
	return (UNKNOWN);
...
#ifdef QUICKDATE
time_t quickdate(p)
/* get a date, assuming it's in RFC822 standard format with no leading day */
{
    char	nmonth[10];
    static char	*monthabbrevs[] =
    {
    ...
    if (sscanf(p, "%d %s %d %d:%d:%d GMT", &day,nmonth,&year,&hh,&mm,&ss) != 6)
	return(UNKNOWN);
    else
    {
	register char **mp;

	for(mp = monthabbrevs; *mp; mp++)
	    if (strcmp(mp[0], nmonth) == 0)
		break;
	if (*mp == (char *)NULL)
	    return(UNKNOWN);
	else
	    return(dateconv(mp - monthabbrevs,
			    day, year, hh, mm, ss, 0, 0, STANDARD));
    }
}
#endif /* QUICKDATE */
